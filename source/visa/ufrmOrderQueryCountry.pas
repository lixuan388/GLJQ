...
uses
  ...
  cxDropDownEdit, cxDBLookupComboBox, dxdbtrel;

type
  TfrmOrderQueryCountry = class(TModuleMDIGirdFrm)
...
var
  frmOrderQueryCountry: TfrmOrderQueryCountry;
  procedure ExecfrmOrderQueryCountry;
implementation
...
procedure ExecfrmOrderQueryCountry;
begin
  if frmOrderQueryCountry=nil then
    frmOrderQueryCountry:=TfrmOrderQueryCountry.Create(Application);
  frmOrderQueryCountry.WindowState:=wsMaximized;
  frmOrderQueryCountry.Show;
end;

procedure TfrmOrderQueryCountry.btn_excelClick(Sender: TObject);
begin
  //inherited;    导出Excel
  ...
  with sd_1 do
  begin
    filename:='订单报表-国家统计'+FormatDateTime('mmddhhnnss',Now);
    if Execute then
    begin
...
{select avg_id_aar,avg_name_aar,avg_id_act,count(0) as _count,sum(avg_num) as _avg_num,sum(avg_amount) as _avg_amount
from avg_visa_group with(nolock)
where avg_status<>'D'
and avg_date_enter between '2014-11-01' and '2014-11-01'
group by avg_id_aar,avg_name_aar,avg_id_act}

procedure TfrmOrderQueryCountry.Btn_QueryClick(Sender: TObject);
var
  vname:string;
begin
  inherited;//查询
  ...
  begin
    ...
    SQL.Text:='select avg_id_aar,avg_name_aar,avg_id_act,count(0) as _count,sum(avg_num) as _avg_num,sum(avg_amount) as _avg_amount '
             +' from avg_visa_group with(nolock) '
             +' where avg_status<>''D'' ';
    ...
    SQL.Add(' group by avg_id_aar,avg_name_aar,avg_id_act ');
    Open;
  end;
end;

procedure TfrmOrderQueryCountry.FormClose(Sender: TObject;
  var Action: TCloseAction);
begin
  inherited;
  FreeAndNil(frmOrderQueryCountry);
end;

procedure TfrmOrderQueryCountry.FormCreate(Sender: TObject);
begin
  inherited;
  //获取状态选项
